        ...
        Just gsEntity -> do
          ...
          gameStateEntity <-
            if status_ == "Paused"
              then do
                let updateEntity =
                      gsEntity & gameStateEntityStatus .~ "Ongoing"
                ...
                return updateEntity
              else -- In any other case (game was Won or Lost) just return the fetched entity
              do return gsEntity
          defaultLayout $ do
            let gameTableId = gameIds
            setTitle $ toHtml $ gameId_ ++ " | Minesweepskell"
  ...
  case gameStateMaybe of
    ...
    Just gameState -> do
      ...
      _ <- case gameStateAfterMove ^. status of
        ...
        _ -> do
          _ <- runDB $ insert $ gameStateToGameStateEntity gameStateAfterMove
