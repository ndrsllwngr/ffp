...
gameStateToGameStateEntity state =
  ...
  where
    boardToRows board_ = map (Row . map (\c -> cellToCellEntity c (state ^. status))) (Data.Matrix.toLists board_)

gameStateEntityToGameState :: GameStateEntity -> Chan ServerEvent -> GameState
gameStateEntityToGameState entity _channel =
  GameState
  ...
  where
    rowsToBoard rows = Data.Matrix.fromLists $ map (map cellEntityToCell . _rowCells) rows
...
cellToCellEntity cell _gameStatus =
  CellEntity
    { _cellEntityCoordX = fst $ cell ^. coordinate,
      ...
      _cellEntityHasBomb = isGameOverStatus _gameStatus && (cell ^. hasBomb), -- if game is over set hasBomb to False
      _cellEntityNeighboringBombs = if isGameOverStatus _gameStatus then cell ^. neighboringBombs else -1, -- if game is over set neighboringBombs to -1
      _cellEntityAssetId = getCellAssetId _gameStatus cell
    }

cellEntityToCell :: CellEntity -> Cell
cellEntityToCell cellEntity =
  Cell
